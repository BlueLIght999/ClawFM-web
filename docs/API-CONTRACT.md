...
Qclaudio 2.0.0 已为携带 Song 的 Socket 事件增加 v2：
  radio:state-v2 / radio:song-change-v2 / radio:queue-update-v2。
  v2 只包含稳定 Song DTO；v1 事件继续双发一个大版本过渡期，供旧客户端使用。
  2.0.0 前端只订阅 v2，不再读取网易云 ar/al/dt 字段。
```
...
// 事件统一信封
{
  "event": "radio:song-change-v2",
  "payload": { "schemaVersion": 2, "song": { /* stable Song DTO */ } },
...
SE4  v2 Song = {id,title,artist,album,durationMs,coverUrl}，禁止增加 ar/al/dt
```
...
当前版本：Qclaudio 2.0.0
  v1 兼容事件：radio:state / radio:song-change / radio:queue-update
  v2 正式事件：radio:state-v2 / radio:song-change-v2 / radio:queue-update-v2
  服务端通过单一 versioned emitter 双发；前端只消费 v2。
```
